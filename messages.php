...
//Taking of friend id
$friendquery = mysql_query("SELECT friend_id FROM wc__friendlists WHERE user_id = '".$myid."' LIMIT 2");
...
while ($regs = mysql_fetch_array($friendquery)) {
    $query = "SELECT user_name, user_surname, user_smallavatar, user_city, user_univer FROM $table WHERE user_id = '".$regs['friend_id']."'";
    $res = mysql_query($query) or die(mysql_error());
    ...
    <a href="http://<?echo $baseurl?>/mesframe.php?id=<?echo $regs['friend_id']?>">
    <div id="wb_avatar"> 
    ...
    <div id="wb_buttons">
    <a href="modules/delete_usermessages.php?id=<?echo $row['post_id'];?>" style="color: #aaaaaa;">Удалить</a>
    </div>
